...
// tuples
let address: [string, number] = ["Superstreet", 89];
...
/**
 * The important thing in **type alias** is even though we are on the right side
 * after ** = ** (equal sign) in
 *
 *    type Complex = {data: number[], output: (all: boolean) => number[]};
 *
 * This is still type creation because we have **type** in front otherwise in
 * typescript and JavaScript the rule always is on the right of the ** = **
 * (equal sign) you define **type** of variable, of properties, or what ever.
 */
// Creating custom Types with Type Aliases | type alias
type Complex = {data: number[], output: (all: boolean) => number[]};
